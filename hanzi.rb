...
def main
      p "main============"
      stocks = File.new('all_stocks.txt')

      stocks_with_daima = File.new('all_stocks_with_daima.txt','w')
      stocks_for_r_symbol = File.new('stocks_for_r_symbol.R','w')

      stocks.each_line { |line| 
            write_into_daima(stocks_with_daima, stocks_for_r_symbol, line)
      }

      stocks.close
      stocks_with_daima.close
      stocks_for_r_symbol.close
      p "OK"
end

def write_into_daima file1, file2, line
      arr = line.split(" ")
...
def get_daima str
      daima = ""
      (0..str.length-1).each{ |index|
            is_find = false
            DATA.each{ |elem|
                  if(elem[1].include?(str[index]))
                        is_find = true
                        daima << elem[0].upcase[0]
                        break
                  end
            }
            daima << str[index] if(!is_find)
      }
      daima
end

def write_into_r_symbol file, line
